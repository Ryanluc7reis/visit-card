...
import GeneratorPageLink from "@/components/generatorpagelink/GenaratorPageLink";
...
export default function ProfilePage() {
  ...
  const linkCurrent = `${URL}/${user}`;

  const getCard = async () => {
    try {
      const response = await axios.get(`${API_URL}/card/${user}`);
    ...
    <>
      {showPopUp && (
        <PopUpMessage error={messageType === "error" ? true : false}>
          ...
          {messageType === "copyLink" && "Link copiado"}
